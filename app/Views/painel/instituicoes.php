								...
								<div class="row align-items-center">
									...
									<div class="col-12 col-md-6">
										...
										<div class="d-flex justify-content-end">
											<div style="margin-left: 5px;"><a href="<?php echo(painel_url('instituicoes/form')); ?>" class="btn btn-sm btn-primary">Novo Registro</a></div>
										</div>
												...
												<table id="example2" class="display nowrap table table-striped table-bordered" style="width:100%">
													<thead>
														<tr>
															...
															<th>Título / CNPJ</th>
															<th style="width: 28%;">Responsável Legal / E-mail</th>
													...
													<?php
														...
														foreach ($rs_list->getResult() as $row) {
															...
															$insti_hashkey = ($row->insti_hashkey);
															$insti_nome = ($row->insti_nome);
															$insti_cnpj = ($row->insti_cnpj);
															$insti_email = ($row->insti_email);
															$insti_resp_nome = ($row->insti_resp_nome);
															$insti_dte_cadastro = fct_formatdate($row->insti_dte_cadastro, 'd/m/Y H:i:s');

															$link_form = painel_url('instituicoes/form/'. $insti_id);
															...
															<tr class="trRow">
																<td class="text-center">
																	<div class="d-flex justify-content-center">
																		...
																		<div style="margin: 0 3px;">
																			<a href="javascript:;" class="btn btn-sm btn-ac btn-danger cmdExcluirRegistro" data-hashkey="<?php echo($insti_hashkey); ?>"><i class="las la-trash"></i></a>
																		</div>
																		<div class="d-none" style="margin: 0 3px;">
																...
																<td><?php echo($insti_id); ?></td>
																<td>
																	<div><?php echo($insti_nome); ?></div>
																	<div><?php echo($insti_cnpj); ?></div>
																</td>
																<td>
																	<div><?php echo($insti_resp_nome); ?></div>
																	<div><?php echo($insti_email); ?></div>
																</td>
																<td><?php echo($insti_dte_cadastro); ?></td>
															</tr>
														<?php
														}
													?>
			...
			$(document).on('click', '.cmdExcluirRegistro', function (e) {
				...
				Swal.fire({
					...
					if (result.value) {
						// ------------------------------------------------------
						let $formData = {
							insti_hashkey: $hashkey
						};

						$.ajax({
							url: painel_url  +'instituicoes/ajaxform/EXCLUIR-REGISTRO',
							method:"POST",
							type: "POST",
							dataType: "json",
							data: $formData,
