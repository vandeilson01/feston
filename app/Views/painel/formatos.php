								...
								<div class="row align-items-center">
									...
									<div class="col-12 col-md-6">
										...
										<div class="d-flex justify-content-end">
											<div style="margin-left: 5px;"><a href="<?php echo(painel_url('formatos/form')); ?>" class="btn btn-sm btn-primary">Novo Registro</a></div>
										</div>
												...
												<table id="example2" class="display nowrap table table-striped table-bordered" style="width:100%">
													<thead>
														<tr>
															...
															<th>Título</th>
															<th class="text-center" style="width:145px;">Nº Min. Participante</th>
															<th class="text-center" style="width:145px;">Nº Máx. Participante</th>
															<th class="text-center" style="width:160px;">Tempo Limite Música</th>
														</tr>
													</thead>
													<tbody>
													<?php
														...
														foreach ($rs_list->getResult() as $row) {
															...
															$formt_hashkey = ($row->formt_hashkey);
															$formt_titulo = ($row->formt_titulo);
															$formt_min_partic = ($row->formt_min_partic);
															$formt_max_partic = ($row->formt_max_partic);
															$formt_tempo_limit = ($row->tempo_formatado);

															$link_form = painel_url('formatos/form/'. $formt_id);
															...
															<tr class="trRow">
																<td class="text-center">
																	<div class="d-flex justify-content-center">
																		...
																		<div style="margin: 0 3px;">
																			<a href="javascript:;" class="btn btn-sm btn-ac btn-danger cmdExcluirRegistro" data-hashkey="<?php echo($formt_hashkey); ?>"><i class="las la-trash"></i></a>
																		</div>
																	</div>
																</td>
																<td><?php echo($formt_id); ?></td>
																<td><?php echo($formt_titulo); ?></td>
																<td class="text-center"><?php echo($formt_min_partic); ?></td>
																<td class="text-center"><?php echo($formt_max_partic); ?></td>
																<td class="text-center"><?php echo($formt_tempo_limit); ?></td>
															</tr>
														<?php
														}
													?>
			...
			$(document).on('click', '.cmdExcluirRegistro', function (e) {
				...
				Swal.fire({
					...
					if (result.value) {
						// ------------------------------------------------------
						let $formData = {
							formt_hashkey: $hashkey
						};

						$.ajax({
							url: painel_url  +'subgeneros/ajaxform/EXCLUIR-REGISTRO',
							method:"POST",
							type: "POST",
							dataType: "json",
							data: $formData,
