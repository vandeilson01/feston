...
class Modalidades extends PainelController
{
	...
	public function filtrar()
	{
		...
		$this->modlMD
			...
			->orderBy('modl_id', 'DESC')
			->limit(1000);
		$query = $this->modlMD->get();

		$this->data['lastQuery'] = $this->modlMD->getLastQuery();
			//->getCompiledSelect();
		...
		return view($this->directory .'/modalidades', $this->data);
	}
	...
	public function form( $modl_id = 0 )
	{
		if ($this->request->getPost())
		{
			...
			if ($this->validate($rules)) {
				$modl_titulo = $this->request->getPost('modl_titulo');

				$data_db = [
					'insti_id' => (int)$this->session_user_id,
					'modl_hashkey' =>md5(date("Y-m-d H:i:s") ."-". random_string('alnum', 16)),
					'modl_urlpage' => url_title( convert_accented_characters($modl_titulo), '-', TRUE ),
					'modl_titulo' => $modl_titulo,
					'modl_dte_cadastro' => date("Y-m-d H:i:s"),
					'modl_dte_alteracao' => date("Y-m-d H:i:s"),
					'modl_ativo' => 1,
				];

				$queryEdit = $this->modlMD->where('modl_id', $modl_id)->get();
				if( $queryEdit && $queryEdit->resultID->num_rows >=1 )
				{
					unset( $data_db['modl_hashkey'] );
					unset( $data_db['modl_dte_cadastro'] );
					$qryExecute = $this->modlMD->update($modl_id, $data_db);
				}else{
					$modl_id = $this->modlMD->insert($data_db);
				}

				return $this->response->redirect( painel_url('modalidades') );
				exit();
		...
		$query = $this->modlMD->where('modl_id', $modl_id)->get();
		if( $query && $query->resultID->num_rows >=1 )
		{
		...
		return view($this->directory .'/modalidades-form', $this->data);
	}
	...
	public function ajaxform( $action = "" )
	{
		...
		case "EXCLUIR-REGISTRO" :
			...
			$modl_hashkey = $this->request->getPost('modl_hashkey');
			$query = $this->modlMD->where('modl_hashkey', $modl_hashkey)->get();
			if( $query && $query->resultID->num_rows >=1 )
			{
				$rs_registro = $query->getRow();
				$modl_id = (int)$rs_registro->modl_id;			

				// excluir registro
				$this->modlMD->where('modl_hashkey', $modl_hashkey)->delete();

				//$this->modlMD->set('solt_excluido', 1);
				//$this->modlMD->where('modl_hashkey', $modl_hashkey);
				//$this->modlMD->where('modl_id', $modl_id);
				//$this->modlMD->update();
