...
class Generos extends PainelController
{
	protected $geneMD = null;

    public function __construct()
    {
        $this->geneMD = new \App\Models\GenerosModel();
	...
	public function filtrar()
	{
		...
		$this->geneMD
			...
			->orderBy('gene_id', 'DESC')
			->limit(1000);
		$query = $this->geneMD->get();

		$this->data['lastQuery'] = $this->geneMD->getLastQuery();
			//->getCompiledSelect();
		...
		return view($this->directory .'/generos', $this->data);
	}
	...
	public function filtrar_old()
	{
		...
		return view('generos', $this->data);
	}


	public function form( $gene_id = 0 )
	{
		if ($this->request->getPost())
		{
			...
			if ($this->validate($rules)) {
				$gene_titulo = $this->request->getPost('gene_titulo');

				$data_db = [
					'insti_id' => (int)$this->session_user_id,
					'gene_hashkey' =>md5(date("Y-m-d H:i:s") ."-". random_string('alnum', 16)),
					'gene_urlpage' => url_title( convert_accented_characters($gene_titulo), '-', TRUE ),
					'gene_titulo' => $gene_titulo,
					'gene_dte_cadastro' => date("Y-m-d H:i:s"),
					'gene_dte_alteracao' => date("Y-m-d H:i:s"),
					'gene_ativo' => 1,
				];

				$queryEdit = $this->geneMD->where('gene_id', $gene_id)->get();
				if( $queryEdit && $queryEdit->resultID->num_rows >=1 )
				{
					unset( $data_db['gene_hashkey'] );
					unset( $data_db['gene_dte_cadastro'] );
					$qryExecute = $this->geneMD->update($gene_id, $data_db);
				}else{
					$gene_id = $this->geneMD->insert($data_db);
				}

				return $this->response->redirect( painel_url('generos') );
				exit();
		...
		$query = $this->geneMD->where('gene_id', $gene_id)->get();
		if( $query && $query->resultID->num_rows >=1 )
		{
		...
		return view($this->directory .'/generos-form', $this->data);
	}
	...
	public function ajaxform( $action = "" )
	{
		...
		case "EXCLUIR-REGISTRO" :
			...
			$gene_hashkey = $this->request->getPost('gene_hashkey');
			$query = $this->geneMD->where('gene_hashkey', $gene_hashkey)->get();
			if( $query && $query->resultID->num_rows >=1 )
			{
				$rs_registro = $query->getRow();
				$gene_id = (int)$rs_registro->gene_id;			

				// excluir registro
				$this->geneMD->where('gene_hashkey', $gene_hashkey)->delete();

				//$this->geneMD->set('solt_excluido', 1);
				//$this->geneMD->where('gene_hashkey', $gene_hashkey);
				//$this->geneMD->where('gene_id', $gene_id);
				//$this->geneMD->update();
