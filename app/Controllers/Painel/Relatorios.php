...
class Relatorios extends PainelController
{
	...
	public function agrupar_old()
	{
		// relatorios
		$_grp_hashkey = (isset($this->rs_params->grp) ? $this->rs_params->grp : "");
		
		return view($this->directory .'/relatorios', $this->data);
	}


	public function agrupar( $corgf_id = 0 )
	{
	...
	public function excel( $corgf_id = 0 )
	{
		...
		header('Content-Disposition: attachment;filename="event_data.xlsx"');
		header('Cache-Control: max-age=0');

		// Criar um objeto de escrita para salvar a planilha
		...
		**/
			...
			*/
			...
			$grp_hashkey = (isset($this->rs_params->grp) ? $this->rs_params->grp : "");
			//self::cobranca_post( 0, $_grp_hashkey);
			...
			**/
				...
				{
					$rs_event = $query_grupo_evt->getRow();

					$insti_id = (int)$rs_event->insti_id;
					$event_id = (int)$rs_event->event_id;
					$event_titulo = $rs_event->event_titulo;
					$grevt_id = (int)$rs_event->grevt_id;
					$grp_id = (int)$rs_event->grp_id;
					$grp_titulo = $rs_event->grp_titulo;

					$grp_responsavel = $rs_event->grp_responsavel;
					$grp_telefone = $rs_event->grp_telefone;
					$grp_celular = $rs_event->grp_celular;
					$grp_cpf = $rs_event->grp_cpf;

					$grp_end_cep = $rs_event->grp_end_cep;
					$grp_end_logradouro = $rs_event->grp_end_logradouro;
					$grp_end_numero = $rs_event->grp_end_numero;
					$grp_end_compl = $rs_event->grp_end_compl;
					$grp_end_cidade = $rs_event->grp_end_cidade;
					$grp_end_estado = $rs_event->grp_end_estado;
					...
					$arr_dados_excel = [
						["label" => "Evento", "value" => $event_titulo],
						["label" => "Nome do Grupo", "value" => $grp_titulo],
						["label" => "Registro Ativo", "value" => "Sim"],
						["label" => "Responsável", "value" => $grp_responsavel],
						["label" => "CPF do Responsável", "value" => $grp_cpf],
						["label" => "Telefone", "value" => $grp_telefone],
						["label" => "Celular", "value" => $grp_celular],
						["label" => "E-mail", "value" => "Sim"],
						["label" => "Instagram", "value" => "Sim"],
						["label" => "Facebook", "value" => "Sim"],
						["label" => "Vimeo", "value" => "Sim"],
						["label" => "CEP", "value" => $grp_end_cep],
						["label" => "Endereço", "value" => $grp_end_compl],
						["label" => "Cidade/Estado", "value" => $end_cidade_uf],
					];

					// Criar a primeira aba para informações do evento
					$eventSheet = $spreadsheet->getActiveSheet();
					$eventSheet->setTitle('Informações do Evento');

					$rowIni = 0;
					foreach ($arr_dados_excel as $key => $val) {
						$rowIni++;
						$eventSheet->setCellValue('A'. $rowIni, $val['label']);
						$eventSheet->setCellValue('B'. $rowIni, $val['value']);
					}

					/*
					$eventSheet->setCellValue('A1', "Evento");
					$eventSheet->setCellValue('B1', $event_titulo);
					$eventSheet->setCellValue('A2', "Nome do Grupo");
					$eventSheet->setCellValue('B2', $grp_titulo);
					$eventSheet->setCellValue('A3', "Registro Ativo");
					$eventSheet->setCellValue('B3', "Sim");
					$eventSheet->setCellValue('B2', 'Data do Evento');
					$eventSheet->setCellValue('C1', 'Local do Evento');
					*/
					...
					/*
					 * -------------------------------------------------------------
					 * EXCEL : ABA : PARTICIPANTES
					 * -------------------------------------------------------------
					**/
						// Criar a segunda aba para participantes
						$participantsSheet = $spreadsheet->createSheet();
						$participantsSheet->setTitle('Participantes');

						// Adicionar cabeçalho das colunas na segunda aba
						$participantsSheet->setCellValue('A1', 'ID');
						$participantsSheet->setCellValue('B1', 'Nome');
						$participantsSheet->setCellValue('C1', 'Documento');
						$participantsSheet->setCellValue('D1', 'Função');
						//$participantsSheet->setCellValue('E1', 'Categoria');

						// Simulação de dados dos participantes (substitua com seus dados reais)
						//$participants = [
						//	['João Silva', 'joao@example.com', '123.456.789-00', 30, 'Categoria A'],
						//	['Maria Oliveira', 'maria@example.com', '987.654.321-00', 25, 'Categoria B'],
						//	['Pedro Santos', 'pedro@example.com', '456.789.123-00', 35, 'Categoria C'],
						//];

						$rowPart = 2;
						$rs_participantes_all = self::fct_elenco_por_grupo( (int)$insti_id, (int)$grp_id );
						foreach ($rs_participantes_all as $row) {
							...
							$participantsSheet->setCellValue('A'. $rowPart, $partc_id);
							$participantsSheet->setCellValue('B'. $rowPart, $partc_nome);
							$participantsSheet->setCellValue('C'. $rowPart, $partc_documento);
							$participantsSheet->setCellValue('D'. $rowPart, $func_titulo);

							//$participantsSheet->fromArray($participant, null, 'A' . $row);
							//$row++;
							//// Adicionar os participantes na segunda aba
							//$row = 2;
			...
			/*
			 * -------------------------------------------------------------
			 * Gerar o Arquivo Excel no diretório de Uploads
			 * -------------------------------------------------------------
			**/
				$file_name = "ficha-de-inscricao-excel___". date("Y-m-d_His") .".xlsx";

				$writer = new Xlsx($spreadsheet);
				$path_file = WRITEPATH ."/uploads/". $file_name;
				$writer->save($path_file);

				if( file_exists($path_file) && !empty($file_name) )
				{	
					// doc_apex___1663959591_1013175868cfc7406fc7.pdf
					return $this->response->download($path_file, null);
				}
		...
		**/
		...
		if( $query_grupo_evt && $query_grupo_evt->resultID->num_rows >=1 )
		{
			...
			**/
				...
				$this->data['rs_participantes_all'] = self::fct_elenco_por_grupo( (int)$insti_id, (int)$grp_id );
				...
				if( $query_corgf_cadastradas && $query_corgf_cadastradas->resultID->num_rows >= 1 )
				{
					...
					$this->data['rs_coreografias_all'] = $query_corgf_cadastradas->getResult();
					$this->data['total_coreografias'] = $query_corgf_cadastradas->resultID->num_rows;
	...
	public function fct_elenco_por_grupo( $insti_id = '', $grp_id = '' )
	{
		$rs_participantes_all = null;
		...
		**/
		$this->crfpaMD->from('tbl_coreografias_x_participantes CRFPA', true)
			->select('PARTC.partc_id, PARTC.partc_hashkey, PARTC.partc_nome, PARTC.partc_documento ')
			->select('FUNC.func_id, FUNC.func_titulo')
			->join('tbl_coreografias CORF', 'CORF.corgf_id = CRFPA.corgf_id', 'INNER')
			->join('tbl_participantes PARTC', 'PARTC.partc_id = CRFPA.partc_id', 'INNER')
			->join('tbl_funcoes AS FUNC', 'FUNC.func_id = PARTC.func_id', 'INNER')
			->where('CORF.insti_id', (int)$insti_id)
			->where('CORF.grp_id', (int)$grp_id)
			->orderBy('PARTC.partc_nome', 'ASC')
			->limit(500);
		$query_elenco = $this->crfpaMD->get();
		//print $this->crfpaMD->getLastQuery();
		if( $query_elenco && $query_elenco->resultID->num_rows >=1 )
		{
			$rs_participantes_all = $query_elenco->getResult();
		}
		return $rs_participantes_all;
	}
